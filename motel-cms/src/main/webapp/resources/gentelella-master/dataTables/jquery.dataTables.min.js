...
{factory(jQuery)}}(function($){"use strict";var DataTable;var _ext;var _Api;var _api_register;var _api_registerPlural;var _re_new_lines=/[\r\n]/g;var _re_html=/<.*?>/g;var _re_formatted_numeric=/[',$£€¥%]/g;var _re_date_start=/^[\d\+\-a-zA-Z]/;var _empty=function(d){return!d||d==='-'?!0:!1};var _intVal=function(s){var integer=parseInt(s,10);return!isNaN(integer)&&isFinite(s)?integer:null};var _isNumber=function(d,formatted){if(formatted&&typeof d==='string'){d=d.replace(_re_formatted_numeric,'')}
    return!d||d==='-'||(!isNaN(parseFloat(d))&&isFinite(d))};var _isHtml=function(d){return!d||typeof d==='string'};var _htmlNumeric=function(d,formatted){if(_empty(d)){return!0}
    var html=_isHtml(d);return!html?null:_isNumber(_stripHtml(d),formatted)?!0:null};var _pluck=function(a,prop,prop2){var out=[];var i=0,ien=a.length;if(prop2!==undefined){for(;i<ien;i++){if(a[i]&&a[i][prop]){out.push(a[i][prop][prop2])}}}
...
{_fnHungarianMap(o[key])}}});o._hungarianMap=map}
    ...
    {oSettings.aoPreSearchCols[iCol]=$.extend(!0,{},DataTable.models.oSearch)}
    ...
    {_fnLog(oSettings,0,"Requested unknown parameter "+(typeof oCol.mData=='function'?'{function}':"'"+oCol.mData+"'")+" for row "+iRow,4);oSettings.iDrawError=oSettings.iDraw}
    ...
    {ajaxData=ajax.data;var newData=$.isFunction(ajaxData)?ajaxData(data):ajaxData;data=$.isFunction(ajaxData)&&newData?newData:$.extend(!0,data,newData);delete ajax.data}
        var baseAjax={"data":data,"success":function(json){var error=json.error||json.sError;if(error){oSettings.oApi._fnLog(oSettings,0,error)}
                oSettings.json=json;_fnCallbackFire(oSettings,null,'xhr',[oSettings,json]);fn(json)},"dataType":"json","cache":!1,"type":oSettings.sServerMethod,"error":function(xhr,error,thrown){var log=oSettings.oApi._fnLog;if(error=="parsererror"){log(oSettings,0,'Invalid JSON response',1)}
    ...
    {oSettings.iDraw++;_fnProcessingDisplay(oSettings,!0);var iColumns=oSettings.aoColumns.length;var aoData=_fnAjaxParameters(oSettings);_fnBuildAjax(oSettings,aoData,function(json){_fnAjaxUpdateDraw(oSettings,json)},oSettings);return!1}
        return!0}
    ...
    {var
        ...
        data.__legacy=!0;return settings.sAjaxSource||DataTable.ext.legacy.ajax?data:d}
    ...
    {var
        ...
        else{var tmpTable=$(table.cloneNode(!1)).css('visibility','hidden').removeAttr('id').append($(oSettings.nTHead).clone(!1)).append($(oSettings.nTFoot).clone(!1)).append($('<tbody><tr/></tbody>'));tmpTable.find('tfoot th, tfoot td').css('width','');var tr=tmpTable.find('tbody tr');headerCells=_fnGetUniqueThs(oSettings,tmpTable.find('thead')[0]);for(i=0;i<visibleColumns.length;i++){column=columns[visibleColumns[i]];headerCells[i].style.width=column.sWidthOrig!==null&&column.sWidthOrig!==''?_fnStringToCss(column.sWidthOrig):''}
        ...
        if(tableWidthAttr){table.style.width=_fnStringToCss(tableWidthAttr);if(!oSettings._reszEvt){$(window).bind('resize.DT-'+oSettings.sInstance,_fnThrottle(function(){_fnAdjustColumnSizing(oSettings)}));oSettings._reszEvt=!0}}}
    ...
    {var api=this.api(!0);if(src!==undefined){var type=src.nodeName?src.nodeName.toLowerCase():'';return col!==undefined||type=='td'||type=='th'?api.cell(src,col).data():api.row(src).data()}
        return api.data().toArray()};this.fnGetNodes=function(iRow)
    {var api=this.api(!0);return iRow!==undefined?api.row(iRow).node():api.rows().nodes().toArray()};this.fnGetPosition=function(node)
    ...
    {return this.api(!0).row(nTr).child(mHtml,sClass).show()};this.fnPageChange=function(mAction,bRedraw)
    ...
    {return function(){var args=[_fnSettingsFromNode(this[DataTable.ext.iApiIndex])].concat(Array.prototype.slice.call(arguments));return DataTable.ext.internal[fn].apply(this,args)}}
        ...
        this.each(function(){var o={};var oInit=len>1?_fnExtend(o,options,!0):options;var i=0,iLen,j,jLen,k,kLen;var sId=this.getAttribute('id');var bInitHandedOff=!1;var defaults=DataTable.defaults;if(this.nodeName.toLowerCase()!='table')
        {_fnLog(null,0,'Non-table node initialisation ('+this.nodeName+')',2);return}
            _fnCompatOpts(defaults);_fnCompatCols(defaults.column);_fnCamelToHungarian(defaults,defaults,!0);_fnCamelToHungarian(defaults.column,defaults.column,!0);_fnCamelToHungarian(defaults,oInit);var allSettings=DataTable.settings;for(i=0,iLen=allSettings.length;i<iLen;i++)
            ...
            var oSettings=$.extend(!0,{},DataTable.models.oSettings,{"nTable":this,"oApi":_that.internal,"oInit":oInit,"sDestroyWidth":$(this)[0].style.width,"sInstance":sId,"sTableId":sId});allSettings.push(oSettings);oSettings.oInstance=(_that.length===1)?_that:$(this).dataTable();_fnCompatOpts(oInit);if(oInit.oLanguage)
            ...
            else{$.extend(oSettings.oClasses,DataTable.ext.classes,oInit.oClasses)}
            ...
            {oSettings.oLanguage.sUrl=oInit.oLanguage.sUrl;$.getJSON(oSettings.oLanguage.sUrl,null,function(json){_fnLanguageCompat(json);_fnCamelToHungarian(defaults.oLanguage,json);$.extend(!0,oSettings.oLanguage,oInit.oLanguage,json);_fnInitialise(oSettings)});bInitHandedOff=!0}
    ...
    {if(typeof selector==='number'){return[a[selector]]}
        ...
        _fnReDraw(settings,holdPosition);if(callback){callback(json)}})}};_api_register('ajax.json()',function(){var ctx=this.context;if(ctx.length>0){return ctx[0].json}});_api_register('ajax.reload()',function(callback,resetPaging){return this.iterator('table',function(settings){__reload(settings,resetPaging===!1,callback)})});_api_register('ajax.url()',function(url){var ctx=this.context;if(url===undefined){if(ctx.length===0){return undefined}
    ...
    {var
        i,ien,tmp,a=[],displayFiltered=settings.aiDisplay,displayMaster=settings.aiDisplayMaster;var search=opts.search,order=opts.order,page=opts.page;if(page=='current')
    ...
    {var table=$(settings.nTable);table.off('draw.DT_details');table.off('column-visibility.DT_details');if(_pluck(settings.aoData,'_details').length>0){table.on('draw.DT_details',function(){table.find('tbody tr').each(function(){var rowIdx=_fnNodeToDataIndex(settings,this);var row=settings.aoData[rowIdx];if(row._detailsShow){row._details.insertAfter(this)}})});table.on('column-visibility.DT_details',function(e,settings,idx,vis){var row,visible=_fnVisbleColumns(settings);for(var i=0,ien=settings.aoData.length;i<ien;i++){row=settings.aoData[i];if(row._details){row._details.children('td[colspan]').attr('colspan',visible)}}})}};_api_register('row().child()',function(data,klass){var ctx=this.context;if(data===undefined){return ctx.length&&this.length?ctx[0].aoData[this[0]]._details:undefined}
        ...
        return!1});var __re_column_selector=/^(.*):(name|visIdx|visible)$/;var __column_selector=function(settings,selector,opts)
    {var
        ...
        else{$(_pluck(settings.aoData,'anCells',column)).remove();col.bVisible=!1;_fnDrawHead(settings,settings.aoHeader);_fnDrawHead(settings,settings.aoFooter);_fnSaveState(settings)}
    ...
    {var data=settings.aoData;var rows=_selector_row_indexes(settings,opts);var cells=_pluck_order(data,rows,'anCells');var allCells=$([].concat.apply([],cells));var row;var columns=settings.aoColumns.length;var a,i,ien,j;return _selector_run(selector,function(s){if(!s){a=[];for(i=0,ien=rows.length;i<ien;i++){row=rows[i];for(j=0;j<columns;j++){a.push({row:row,column:j})}}
        ...
        $.extend(preSearch[column],{"sSearch":input+"","bRegex":regex===null?!1:regex,"bSmart":smart===null?!0:smart,"bCaseInsensitive":caseInsen===null?!0:caseInsen});_fnFilterComplete(settings,settings.oPreviousSearch,1)})});DataTable.versionCheck=DataTable.fnVersionCheck=function(version)
    ...
    {var t=$(table).get(0);var is=!1;$.each(DataTable.settings,function(i,o){if(o.nTable===t||o.nScrollHead===t||o.nScrollFoot===t){is=!0}});return is};DataTable.tables=DataTable.fnTables=function(visible)
